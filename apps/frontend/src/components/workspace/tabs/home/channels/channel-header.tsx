import React from "react";

import { FileText, Mail, MessageCircle, UserPlus } from "lucide-react";

import { Button } from "@/components/ui/button";
import { WorkspaceChannel } from "@/prisma-types/client";

import ChannelInviteDialog from "./channel-invite-dialog";
import ChannelMenuDialog from "./menu/channel-menu-dialog";

const tabs = [
    { id: "messages", label: "Messages", icon: MessageCircle },
    { id: "files", label: "Files", icon: FileText },
];

function ChannelHeader({
    workspaceChannelData,
    activeTab,
    setActiveTab,
}: Readonly<{
    workspaceChannelData: WorkspaceChannel;
    activeTab: string;
    setActiveTab: (tab: string) => void;
}>) {
    return (
        <div className="flex w-full flex-col space-y-2 border-b bg-background px-4 pt-4 md:px-6">
            <div className="flex justify-between">
                <div className="flex items-center justify-between">
                    <h2 className="text-lg font-semibold text-foreground md:text-xl">
                        #{workspaceChannelData.name}
                    </h2>
                </div>
                <div className="flex items-center gap-2">
                    <ChannelInviteDialog>
                        <Button variant={"outline"} size={"sm"} className="rounded-md px-3 py-1">
                            <UserPlus size={15} />
                            Invite Teammates
                        </Button>
                    </ChannelInviteDialog>
                    <ChannelMenuDialog workspaceChannelData={workspaceChannelData} />
                </div>
            </div>

            <div className="mt-3 flex gap-4 overflow-x-auto md:gap-6">
                {tabs.map((tab) => {
                    const Icon = tab.icon;
                    return (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`flex cursor-pointer items-center gap-2 border-b-2 pb-2 text-sm whitespace-nowrap transition-colors ${
                                activeTab === tab.id
                                    ? "border-btn-color text-foreground"
                                    : "border-transparent text-muted-foreground hover:text-foreground"
                            }`}
                        >
                            <Icon size={16} />
                            <span className="hidden sm:inline">{tab.label}</span>
                        </button>
                    );
                })}
            </div>
        </div>
    );
}

export default ChannelHeader;
