...
import { DocumentViewerDialog, OFFICE_TYPES, getOriginalFile } from "../channels/messages/attachment";

const DirectMessageFiles = ({ directId }: { directId: string }) => {
    ...
    return (
        <div className="mx-4 my-2 mb-10 flex-1 text-foreground">
            {isLoading ? (
                <div className="flex h-full h-screen w-full flex-1 items-center justify-center">
                ...
                <div className="mb-12 overflow-y-auto">
                    ...
                    <Card className="w-full gap-0 border-border bg-card px-1 py-1">
                        ...
                        {messages?.messages?.map((message) => (
                            <DirectMessageFileItem key={message.id} message={message} />
                        ))}
                    </Card>
                </div>
            )}
...
export const DirectMessageFileItem = ({ message }: { message: MessageType }) => {
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-muted-foreground text-xs font-medium">{message.senderUser.user.name}</span>
                        <span className="text-muted-foreground/60 text-[10px]">•</span>
                        <span className="text-muted-foreground/80 text-xs">
                            {format(message.createdAt, "MMM dd, yyyy 'at' h:mm a")}
                        </span>
                    </div>
                </div>
            </div>
...
export default DirectMessageFiles;
